...
describe("Cache", () => {
  ...
  describe("scoped", () => {
    it("returns a scoped cache", () => {
      const cache = new Cache({
        global: { testing: "Hello world" },
      }).scoped("global");

      cache.update("testing", (v) => v.toUpperCase());

      expect(cache.get("testing"), "to equal", "HELLO WORLD");
    });
    ...
    it("supports subscribe", () => {
      ...
      cache.subscribe("testing", spy);

      cache.set("testing", "Hello beautiful world");

      expect(cache.get("testing"), "to equal", "Hello beautiful world");
    ...
    it("supports scoping a scoped cache", () => {
      const cache = new Cache({
      ...
      cache.update("level3", (v) => v.toUpperCase());

      expect(cache.get("level3"), "to equal", "HELLO WORLD");
    });
  });
